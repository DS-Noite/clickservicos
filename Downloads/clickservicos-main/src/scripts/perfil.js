...
function enviarPedido() {
  const nomeProf = document.getElementById('idName').textContent;
  
  const pedido = {
    profissional: nomeProf,
    servico: document.getElementById('pedidoDetalhes').value,
  ...
  let pedidos = JSON.parse(localStorage.getItem("cs_solicitacoes") || "[]");
  pedidos.push(pedido);
  localStorage.setItem("cs_solicitacoes", JSON.stringify(pedidos));

  alert("Pedido enviado com sucesso para " + nomeProf + "!");
  
  // Limpar os campos após enviar
  document.getElementById('pedidoDetalhes').value = '';
  document.getElementById('pedidoData').value = '';
  document.getElementById('pedidoBairro').value = '';
  document.getElementById('pedidoCidade').value = '';
  document.getElementById('pedidoRua').value = '';
